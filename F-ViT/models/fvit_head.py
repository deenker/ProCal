...
class FViTBBoxHead(ConvFCBBoxHead):
    ...
    def forward(self, x, vlm_box_feats=None, PENALTY=None):
        ...
        if not self.training and normalized_vlm_box_feats is not None:
            ...
            if PENALTY is not None:
                loc_score = (PENALTY['bg_bnk_sim'] - PENALTY['bnk_sim'])
                bg_score = -1*(PENALTY['bnk_sim4'])

                ################################################
                # score fusion for proposal prior
                temperature1 = 50
                temperature2 = 10
                lambda_ = 0.5
                ### geometric mean
                # loc_score = torch.sigmoid(loc_score * temperature1)
                # bg_score = torch.sigmoid(bg_score * temperature2)
                # pen12_score = loc_score ** lambda_ * bg_score ** (1 - lambda_)
                ###  arithmetic mean
                pen12_score = torch.sigmoid(loc_score * temperature1 * lambda_ + bg_score * temperature2 * (1 - lambda_))
                ##################################################
                gamma = args.gamma
                
                cls_score[:, self.novel_idx] = cls_score[:, self.novel_idx] ** (
                        1 - gamma) * (pen12_score ** gamma)
...
class FViTRoIHead(StandardRoIHead):
    ...
    def simple_test(self,
        ...
        det_bboxes, det_labels = self.simple_test_bboxes(
            x, img_metas, proposal_list, self.test_cfg, rescale=rescale, vlm_feat=vlm_feat, PENALTY=PENALTY)
    ...
    def simple_test_bboxes(self,
        ...
        bbox_results = self._bbox_forward(x, rois, vlm_feat=vlm_feat, PENALTY=PENALTY)
    ...
    def _bbox_forward(self, x, rois, vlm_feat=None, PENALTY=None):
        ...
        cls_score, bbox_pred = self.bbox_head(bbox_feats, vlm_roi_feats, PENALTY=PENALTY)
